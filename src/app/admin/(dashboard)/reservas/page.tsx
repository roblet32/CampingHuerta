        ...
        <div className="space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 w-full">
                    ...
                    <Link
                        ...
                        className="w-full sm:w-auto px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-bold hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-100 active:scale-95"
                    >
                                        ...
                                        <tr key={res.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col gap-1.5">
                                                    <span className={`w-fit px-2.5 py-1 rounded-full text-xs font-semibold border ${statusLabels[res.status]?.color || statusLabels.PENDING.color}`}>
                                                        {statusLabels[res.status]?.label || res.status}
                                                    </span>
                                                    {res.status === 'PAID' && (() => {
                                                        const stay = getStayStatus(res.startDate, res.endDate, res.status);
                                                        return (
                                                            <span className={`w-fit px-2.5 py-0.5 rounded-full text-[10px] uppercase tracking-wider font-bold border shadow-sm ${stay.color}`}>
                                                                {stay.label}
                                                            </span>
                                                        );
                                                    })()}
